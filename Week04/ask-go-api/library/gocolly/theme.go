...
// SelectTheme Select Category
func SelectTheme(conn *gredis.Conn) {
	list, _ := dao.Theme.Where("id<16207").Order("id desc").All()
	var page uint
	var total = 0
	for k, v := range list {
		/*if v.TotalPage < 2 {
		...
		fmt.Println("Theme:", v, " k:", k)
		page = v.CurrentPage
		for ; page <= v.TotalPage; page++ {
			ThemeColly(conn, uint(v.Id), page, v.SourceUrl)
...
// ThemeColly List Colly
func ThemeColly(conn *gredis.Conn, id, page uint, URL string) {
	...
	c.OnHTML(".listleft", func(e *colly.HTMLElement) {
		e.ForEach(".p30", func(i int, element *colly.HTMLElement) {
			...
			if v.IsNil() || v.IsEmpty() {
				...
				var themeMap = new(model.ThemeMap)
				themeMap.ArticleId = article.Id
				themeMap.ThemeId = uint64(id)
				themeMap.AppId = 0
				dao.ThemeMap.Data(themeMap).Unscoped().OmitEmpty().Insert()
